...
trait Tables extends MySQLDBImpl {
  ...
  case class TmSessionRow(token: String, clientId: Byte, ip: Long, deviceType: Byte, fingerPrint: String, status: Byte, memberId: Long, identity: String, identityTicket: String, gmtCreate: java.sql.Timestamp, gmtUpdate: java.sql.Timestamp)
  ...
  implicit def GetResultTmSessionRow(implicit e0: GR[String], e1: GR[Byte], e2: GR[Long], e3: GR[java.sql.Timestamp]): GR[TmSessionRow] = GR {
    prs => import prs._
      TmSessionRow.tupled((<<[String], <<[Byte], <<[Long], <<[Byte], <<[String], <<[Byte], <<[Long], <<[String], <<[String], <<[java.sql.Timestamp], <<[java.sql.Timestamp]))
  }
  ...
  class TmSession(_tableTag: Tag) extends profile.api.Table[TmSessionRow](_tableTag, Some("sso"), "tm_session") {
    def * = (token, clientId, ip, deviceType, fingerPrint, status, memberId, identity, identityTicket, gmtCreate, gmtUpdate) <> (TmSessionRow.tupled, TmSessionRow.unapply)

    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(token), Rep.Some(clientId), Rep.Some(ip), Rep.Some(deviceType), Rep.Some(fingerPrint), Rep.Some(status), Rep.Some(memberId), Rep.Some(identity), Rep.Some(identityTicket), Rep.Some(gmtCreate), Rep.Some(gmtUpdate)).shaped.<>({ r => import r._; _1.map(_ => TmSessionRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get, _8.get, _9.get, _10.get, _11.get))) }, (_: Any) => throw new Exception("Inserting into ? projection not supported."))
