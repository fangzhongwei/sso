...
import com.jxjxgo.common.helper.{RegHelper, TokenHelper}
...
trait SessionService {
  ...
  implicit def sessionCache2Response(s: SessionCache): SessionResponse = {
    SessionResponse("0", s.clientId, s.version, s.ip, s.deviceType, s.fingerPrint, s.token, s.status, s.memberId, s.identity, s.identityTicket, s.gmtCreate, s.gmtCreate)
  }
}

class SessionServiceImpl @Inject()(sessionRepository: SessionRepository, redisClientTemplate: RedisClientTemplate, memberClientService: MemberEndpoint[Future]) extends SessionService {
  ...
  override def createSession(traceId: String, request: CreateSessionRequest): SessionResponse = {
    val memberResponse: MemberResponse = Await.result(memberClientService.getMemberById(traceId, request.memberId))
  ...
  def checkAndResponse(traceId: String, r: CreateSessionRequest, m: MemberResponse): SessionResponse = {
    m.status match {
      ...
      case _ =>
        if (0 == m.status) Await.result(memberClientService.updateMemberStatus(traceId, m.memberId, 1))
  ...
  def doLogout(traceId: String, token: String): scala.concurrent.Future[Unit] = {
